...
var PinchZoom = /** @class */ (function (_super) {
  ...
  PinchZoom.prototype._computeInitialOffset = function () {
    ...
    var _a = this._getChildSize(),
      width = _a.width,
      height = _a.height;
    var initialZoomFactor = this._getInitialZoomFactor();
    if (this.props.computeInitialOffset) {
      this._initialOffset = this.props.computeInitialOffset(
        rect,
        width,
        height,
        initialZoomFactor,
  ...
  PinchZoom.prototype._defaultComputeInitialOffset = function (
    rect,
    width,
    height,
    initialZoomFactor,
  ) {
    var x = -abs(width * initialZoomFactor - rect.width) / 2;
    var y = -abs(height * initialZoomFactor - rect.height) / 2;
    return { x: x, y: y };
  };
